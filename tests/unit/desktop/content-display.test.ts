...
describe("★ 表情标签：认识的转 emoji，不认识的保留", () => {
  /**
   * ★ 这一组的意图**变过一次**，值得记下来。
   *
   * 原来是"表情标签一律保留"——理由是"用户本来就这么看到"。
   * 但那个理由站不住：用户在钉钉 App 里看到的是**真表情**，
   * 我们显示 `[捂脸哭]` 恰恰与他习惯的界面不一致。
   *
   * 现在的形态是白名单（见 `channel-emoji.ts` 与 `channel-emoji.test.ts`）：
   * 认识的转成 emoji，不认识的原样留着 —— 后者尤其重要，
   * 因为 610 种 `[xx]` 里大量不是表情（告警状态词、表单字段名、人名）。
   */
  it.each([
    ["[捂脸哭]", "🤦"],
    ["[一脸苦笑]", "😅"],
  ])("%s 转成 %s（与钉钉 App 一致）", (marker, emoji) => {
    expect(toDisplayContent(`好的${marker}`).text).toBe(`好的${emoji}`)
  })

  /**
   * ★ 不在白名单里的照旧保留。
   *
   * `[二哈]` 是平台自有形象（没有对应 emoji，刻意不收）；
   * `[向上]` 更像排版符号。两者都属于"留着比错转好"。
   */
  it.each(["[二哈]", "[向上]"])("%s 不在白名单里 → 不动", (marker) => {
    expect(toDisplayContent(`好的${marker}`).text).toBe(`好的${marker}`)
  })
