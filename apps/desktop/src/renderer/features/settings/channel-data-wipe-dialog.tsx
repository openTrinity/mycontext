 ...
 * 点错了最多白跑一轮。而这个按钮把整个渠道身份**归零** —— 几万条消息、
 * 索引、图谱、下载的媒体、**以及渠道凭据**，全部不可逆。所以：
 *
 * · **先预演再问** —— 打开时先跑一次 `dryRun`，把真实条数摆出来。
 *   写死一句"将删除全部数据"是不够的：用户无法判断"全部"是 3 条还是 8 万条，
 *   而那个数量级恰好决定他要不要停手；
 * · **写清清完之后要重新做什么** —— 这个动作会**退出已授权状态**，
 *   下次得重新扫码、重新确认身份、重新选学习范围、重新建数字人身份。
 *   不写清的话用户会以为只是删了点数据，而代价比那大得多；
    ...
    <Dialog open={open} onClose={onCancel} labelledBy={titleId} className="radius-xl">
      ...
      >
        ...
        */}
        ...
        {/*
          ★ 「清完之后要重新做什么」—— 这一段是框里最重要的信息。
          归零会退出授权，用户必须提前知道代价（要重新扫码、重新确认身份、
          重新选范围、重新建数字人）。不写清的话他会以为只是删了点数据。
        */}
