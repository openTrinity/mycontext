...
const ALLOWLIST = new Set([
  ...
  "前向",
  /**
   * ★ 这六个是**同一批**两字假阳性：普通中文词或跨词切片，恰好也是本机某
   * vault 里的两字花名。判据同上："换个人也一样"。
   *
   * 逐个复核（下一个人要能重做 —— `git grep -n <词> -- '*.ts' '*.tsx'`）：
   *
   * · `照常` —— 副词。`post-auth-identity.ts:265`「采集**照常**跑（它不依赖
   *   身份）」、`startup.ts:1424`「页面**照常**降级」。本仓库中文注释里
   *   「照常」是描述降级行为的常用词。
   * · `留白` —— 排版术语。`primitives.tsx:14`「不填满槽位，剩下的**留白**」、
   *   `auth-view.tsx:75`「留下一圈无用**留白**」。
   * · `凌晨` —— 时间词。`message-time.ts:120`「次日**凌晨**的消息显示成
   *   '今天'」、`welcome-header.tsx:99`「**凌晨** 5 点算'早上'」。
   * · `得开` —— **跨词切片**，与上面 `以行`/`起起` 同类。`palette.ts:91`
   *   「与四个 slot 都拉**得开**」——「拉得开」的后两字。
   * · `是仪` —— 同样是跨词切片。`data-plane-panel.tsx:217`「这一页不**是仪**
   *   表盘」——「不是」+「仪表盘」的相邻两字。
   * · `时至` —— 仓库里**一处都没有**（只存在于本机 vault）。留在白名单里是
   *   为了让这一批的复核记录完整：它与上面五个来自同一次扫描，
   *   下次若有人在注释里写下「**时至**今日」不该被当成泄漏。
   *
   * ★ 这批的存在再次说明 `CJK_NAME_MIN_LENGTH = 2` 的代价：两字中文的
   * 撞词率很高。真正的泄漏是长串（`cid…` / `msg…` / 三字以上姓名），
   * 那些不会走到这里 —— 所以放宽两字词的判据不削弱这道门禁的核心作用。
   */
  "照常",
  "留白",
  "凌晨",
