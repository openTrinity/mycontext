...
| Extraction cache | Validated Phase-B LLM extraction output in separate `extraction_cache.db` | Entity/fact construction and structural-edge creation | Chunk/model miss; prompt/schema changes currently require an explicit clear or model-id change | Keyed by chunk id and model-scoped on read; the former `knowledge.db` table is imported once | Preserved by `--fresh-db`; rolling LRU eviction keeps at most `pipelines.ingestion.extraction.cache_max_entries` rows, so the limit must cover the largest extraction workset and an older evicted chunk must be re-extracted during a later rebuild |
