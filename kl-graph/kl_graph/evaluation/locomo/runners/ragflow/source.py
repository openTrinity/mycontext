"""Read native LoCoMo data without creating a second dataset format."""
...
DEFAULT_ARTIFACT_ROOT = PROJECT_ROOT / "data" / "locomo-ragflow"
...
def samples_by_id(samples: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
...
def render_transcript(sample: dict[str, Any]) -> str:
    """Serialize one whole conversation; RAGFlow still owns all chunking."""
    ...
    for session_key in session_keys(conversation):
        session_number = int(_SESSION_KEY.fullmatch(session_key).group(1))
        session_date = str(
