"""Score retrieval and generated answers from a Khoj LoCoMo run."""
...
def _dia_ids(items: list[dict[str, Any]], sample_id: str) -> list[str]:
    """Resolve evidence only from chunks owned by the question conversation."""
    values: list[str] = []
    seen: set[str] = set()
    for item in items:
...
def _retrieval_row(
    ...
    dia_ids = _dia_ids(items, sample_id)
...
def _retrieval_report(
    ...
    return {
        ...
        "parameters": {
            ...
            "dedupe": run.get("dedupe"),
            "content_type": run.get("content_type"),
            "chunking_owner": run.get("chunking_owner"),
        },
...
def _resolve_output_dir(args: argparse.Namespace, run: dict[str, Any]) -> Path:
    ...
    root = root / "benchmark" / "locomo-khoj-score" / category_name
...
def _load_answers(
    ...
    if recorded != ask_dir:
        raise ValueError("answer run does not belong to this Khoj ask run")
    return {
        str(row["id"]): row
...
def main(argv: list[str] | None = None) -> int:
    ...
    if run.get("backend") != "khoj" or run.get("benchmark") != "locomo":
        raise ValueError("--ask-dir is not a Khoj LoCoMo run")
    if run.get("status") != "complete":
        raise RuntimeError(f"Khoj ask run is not complete: {ask_dir}")
