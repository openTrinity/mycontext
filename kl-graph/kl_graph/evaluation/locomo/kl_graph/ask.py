...
def _runtime_options(
    ...
    *,
    ...
    return argparse.Namespace(
        build_configuration_sha256=None,
...
async def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if args.top_k > RERANK_TOP_K:
            raise ValueError(
                f"ask.top_k={args.top_k} exceeds engine result cap {RERANK_TOP_K}"
            )
...
def _select_rows(
    ...
    *,
    ...
    rows = [row for row in rows if row["conversation_id"] in conversations]
    if categories:
    ...
    if question_ids:
        ...
        unknown = sorted(question_ids - found)
        if unknown:
