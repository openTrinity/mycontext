...
def _ok_response(**overrides) -> dict:
    ...
    resp = {
        "answer": "最近你主要在做数据同步评审 [Data: Communities (L1-12, L2-3)]",
        "user": "孙亮",
        "entity_id": "ent-abc",
        "reason": "ok",
        "communities": [
...
def test_explicit_user_wins_and_dws_not_called(
    ...
    result = runner.invoke(
        kl_cli.cli, ["global-search", "我最近的任务是什么", "--user", "张伟"]
    )
    assert result.exit_code == 0, result.output
    method, endpoint, _ = capture.calls[0]
    assert method == "POST"
    assert endpoint == "/global_search"
    assert capture.body == {"query": "我最近的任务是什么", "user": "张伟"}
...
def test_dws_success_sends_org_username(
    ...
    assert result.exit_code == 0, result.output
    assert capture.body == {"query": "我最近的任务是什么", "user": "孙亮"}
...
def test_happy_path_rendering(
    ...
    result = runner.invoke(kl_cli.cli, ["global-search", "我最近的任务是什么"])
    assert result.exit_code == 0, result.output
    assert result.output.index("最近你主要在做数据同步评审") < result.output.index(
        "[ok]"
    ), "answer must render before the metadata block"
    assert "user: 孙亮 (entity: ent-abc)" in result.output
