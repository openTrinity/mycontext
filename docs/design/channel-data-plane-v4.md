...
> **状态：设计。** 本文是 v3 落地之后暴露出的**架构性**问题的解法 ——
> 不是补缺口，而是把一处**分层错误**改对。
>
...
| `purge*` 简化成只在"明确收窄"时跑（§3.3）                            | `purge-scope.ts` 调用点                     |
| 快照：`producers` 那几行的 dropped 改成"打标为 0 的条数"             | `producer-view.ts`                          |
...
| D    | 给 persona 也加 `requires: "attention"`        | ★「关掉会话后不许继续起草」转红（§6.2 那个洞） |
...
- **§4.2 那个"几十毫秒"没有实测** —— 从 `persist` 返回到 `runPull` 末尾
  ...
  ★ 阶段 C 应当先测一次这个间隔（一条日志即可），再决定要不要保留一个纯
  `wake()` 的 emit；
- **`(domain, eligibility, seq)` 这个索引顺序没有实测** —— 它基于
  "eligibility 是低基数等值前缀"这个推断。阶段 D 应当在真实数据量上
  `EXPLAIN QUERY PLAN` 一次；
- **阶段 A 的收窄确认没有交互稿** —— "需要手动重建图谱"这句话要放在
  保存前的确认里还是保存后的提示里，是一个产品决定；
- **`learning_eligible` 的重算成本没有实测** —— 范围放宽时要
  `UPDATE messages SET learning_eligible = …`。本机 84,325 条消息，
  走 `(channel_id, sent_at)` 索引应当是亚秒级，但**没测**。
